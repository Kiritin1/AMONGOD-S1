...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Nykoh Selfbot v{SELFBOT.__version__}] | Loading...')
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}Nykoh {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Nykoh.user.name}#{Nykoh.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Nykoh.user.id}   
                       ...
                       {Fore.CYAN}Creator: {Fore.GREEN}!                の Nykoh 软#0337
    '''+Fore.RESET)
...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            Nykoh.run(token, bot=False, reconnect=True)
            os.system(f'title (Nykoh Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Nykoh = discord.Client()
Nykoh = commands.Bot(
    description='Nykoh Selfbot',
    command_prefix=prefix,
    self_bot=True
)
Nykoh.remove_command('help') 
...
async def btc_status():
    ...
    await Nykoh.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await Nykoh.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{Nykoh.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await Nykoh.process_commands(message)
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Nykoh Selfbot v{SELFBOT.__version__}] | Logged in as {Nykoh.user.name}')
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'Nykoh-Was-Here@Fuckyou.com';
...
@Nykoh.command(aliases=['shorteen'])
...
@Nykoh.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@Nykoh.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Nykoh.command(aliases=['pfp', 'avatar'])
...
@Nykoh.command(name='1337-speak', aliases=['1337speak'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:      
             await Nykoh.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Nykoh.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await Nykoh.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Nykoh.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
@Nykoh.command(aliases=['dong', 'penis'])
...
@Nykoh.command(aliases=['changehypesquad'])
...
@Nykoh.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@Nykoh.command(aliases=['fakeconnection', 'spoofconnection'])
...
@Nykoh.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await Nykoh.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Nykoh.guilds:
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://Nykoh.wtf",
            reason="https://Nykoh-selfbot.github.io",
            icon=None,
            banner=None
        )  
    except:
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = Nykoh.get_user(user.id)
    if ctx.author.id == Nykoh.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@Nykoh.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
@Nykoh.command(aliases=['rainbow-role'])
...
@Nykoh.command(aliases=['slots', 'bet'])
...
@Nykoh.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = Nykoh.get_channel(int(channelid))
...
@Nykoh.command(aliases=['guildpfp'])
...
@Nykoh.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in Nykoh.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)   
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in Nykoh.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )

@Nykoh.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Nykoh.command(aliases=['ethereum'])
...
@Nykoh.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Nykoh.user).map(lambda m: m):
        try:
           await message.delete()
        except:
            pass

@Nykoh.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in Nykoh.private_channels:
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://Nykoh-selfbot.github.io/commands'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
...
async def stream(ctx, *, message): # b'\xfc'
    ...
    await Nykoh.change_presence(activity=stream)    
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await Nykoh.change_presence(activity=game)

@Nykoh.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await Nykoh.change_presence(
...
@Nykoh.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in Nykoh.guilds:
        await guild.ack()
...
@Nykoh.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@Nykoh.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
@Nykoh.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()   
    btc_status.start()        

@Nykoh.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)

@Nykoh.command(aliases=['clearconsole', 'consoleclear'])
...
@Nykoh.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
